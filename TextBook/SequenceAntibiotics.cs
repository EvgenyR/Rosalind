...
namespace TextBook
{
    ...
    public static class SequenceAntibiotics
    {
        ...
        public static string CyclopeptideSequencing(string s)
        {
            List<int> spectr = Helper.ParseIntStringToUnique(s);
            List<int> matrix = Helper.ParseIntString(s);
            List<int> acids = new List<int>();

            foreach (int value in spectr)
            {
            ...
            List<List<int>> prevList = new List<List<int>>(result);

            while (true)
            {
                int summ = prevList[0].Sum();
                if (summ == spectr.Max()) break;

                List<List<int>> newList = new List<List<int>>();
                foreach (List<int> arr in prevList)
                {
                    foreach (int value in acids)
                    {
                        List<int> toAdd = new List<int>(arr);
                        toAdd.Add(value);
                        if (spectr.Contains(toAdd.Sum()) && PeptideHelper.IntOccurence(toAdd, matrix)) newList.Add(toAdd);
                    }
                }

                List<List<int>> trimmedList = new List<List<int>>();
                foreach (List<int> arr in newList)
                {
                    int sum = arr.Sum();
                    if (spectr.Contains(sum) && PeptideHelper.IsValidSpectrum(arr, spectr, false))
                    {
                        trimmedList.Add(arr);
                    }
                }

                prevList = new List<List<int>>(trimmedList);
            }

            List<List<int>> finalList = new List<List<int>>();

            foreach (List<int> lst in prevList)
            {
                if (lst.Sum() == spectr.Max() && PeptideHelper.IsValidSpectrum(lst, spectr, true)) finalList.Add(lst);
            }

            string sresult = string.Empty;

            foreach (List<int> lst in finalList)
            {
                for (int i = 0; i < lst.Count; i++)
                {
                    sresult = sresult + lst[i];
                    if (i != lst.Count - 1) sresult = sresult + "-";
                }
                sresult = sresult + " ";
            }
        ...
        //6. Spectrum convolution problem
