...
namespace Rslnd
{
    ...
    class MyPrefixMatcher<V> : IMyPrefixMatcher<V> where V : class
    {

        private MyTrieNode<V> root;

        private MyTrieNode<V> currMatch;

        private string prefixMatched;

        public bool StringMatcher(string s)
        {
            ResetMatch();
            foreach (char c in s)
            {
                if (currMatch.NumChildren() == 0) return true;
        ...
        public MyPrefixMatcher(MyTrieNode<V> root)
        {
            this.root = root;
            this.currMatch = root;
        }
